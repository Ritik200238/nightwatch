...
def cmd_calibration(args: argparse.Namespace, settings: Settings) -> int:
    ...
    with Store(settings.db_path) as store:
        ...
        if ev:
            print(
                f"\nTAIL ADJUSTMENT (expanding-window, out-of-sample on {ev.n_evaluated} forecasts; last k_lo {ev.k_lo_last:.2f}, margin {ev.c_lo_last or 0:.1f} pts, k_hi {ev.k_hi_last:.2f})\n"
