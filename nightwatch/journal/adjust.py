...
def evaluate_expanding(forecasts: pd.DataFrame, *, min_fit_n: int = MIN_FIT_N, refit_every: int = 25) -> AdjustedEvaluation | None:
    ...
    raw_band = 1.0 - raw_lo - raw_hi
    adj_band = 1.0 - adj_lo - adj_hi
    ...
    return AdjustedEvaluation(
        ...
        raw_band_coverage=raw_band, adj_band_coverage=adj_band, raw_tail_band=raw_tail, adj_tail_band=adj_tail,
        raw_width=float((e["p95"] - e["p5"]).mean()), adj_width=float((e["a95"] - e["a5"]).mean()),
        k_lo_last=last.k_lo if last else None, k_hi_last=last.k_hi if last else None,
