...
def render_text(r: AnalysisReport) -> str:
    ...
    if a is None or not a.result.ok:
        ...
        for name, h in a.horizons.items():
            ...
            lines.append(f"  {name:>10} ({h.hours:.0f}h): n={c.n} mean {c.mean_pct:+.2f}% [{c.ci_mean.low:+.2f},{c.ci_mean.high:+.2f}] med {c.median_pct:+.2f}% win {c.win_rate:.0%} | p5 {c.p5:+.2f}% p95 {c.p95:+.2f}% | worst-in-window p5 {c.mae_p5_pct:+.2f}% | tags {c.tag_counts}{base}")
    lines.append("")
    st = r.stress
